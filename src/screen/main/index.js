...
const Index = ()=>{
    ...
    const loadData =async () =>{
        ...
        var  sorted = result.filter(item=>!(item.proposal && item.proposal.find(item=>item===state.profile.id)))
        sorted = sorted.filter(item=>!(item.invited && item.invited.find(item=>item===state.profile.id)))
        const sortedProposed = result.filter(item=>(item.proposal && item.proposal.find(item=>item===state.profile.id)))
